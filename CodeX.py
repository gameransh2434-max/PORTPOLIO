...
async def on_guild_join(guild: discord.Guild):
    ...
    if log_channel:
        await log_channel.send(f"ITACHI has been added to the server: **{guild.name}** (ID: `{guild.id}`)")

@client.event
async def on_command_completion(context: commands.Context) -> None:
    if context.author.id == 861777169863409715:
        return
    ...
    async with aiohttp.ClientSession() as session:
        ...
        embed.set_footer(text="MADARAX Development™ ❤️", icon_url=client.user.display_avatar.url)
...
@app.route('/')
def home():
    return f"MADARAX Development™ 2025"
